...
public class HSystem {
	static Element globalElements[] = new Element[100];
...
// R1
	...
	public Element[] getElements(){
		Element new_elements[] = new Element[i];
		for(int j = 0; j<i; j++){
...
// R4
	...
	public void simulate(SimulationObserver observer){
		...
		PrintingObserver printingObserver = (PrintingObserver) observer;
...
// R6
	...
	public String layout(){
		...
		for(int i = 0; i< 100; i++){
			...
			for(int j = 0; j<100; j++){
				if(Layout[i][j] != null){
					if(Layout[i][j].contains("MultiSplit")){
						layoutString += Layout[i][j] + " +-> ";
						realCount = count + Layout[i][j].length();
						continue;
					}
					layoutString += Layout[i][j] + " -> ";
					count += Layout[i][j].length() + 4;
				}
			}
			if(realCount != 0){
				layoutString +="\n " + " ".repeat(realCount) +  "|" + "\n" + " ".repeat(realCount+1) + "+-> ";
...
// R7
	...
	public boolean deleteElement(String name) {
		...
		for(Element el: elements){
			...
			if(el.output[0].name.contains(name)){
				if(el.getClass().getSimpleName().equals("Split") || el.getClass().getSimpleName().equals("MultiSplit") ){
					return false;
				}
				else{
					el.output[0] = el.output[0].output[0];
				}
			}
		}
