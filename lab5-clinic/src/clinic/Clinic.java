...
public class Clinic {
	...
	public int loadData(Reader reader) throws IOException {
		...
		for (String s : dataSet) {
			try {
				...
				if (curData[0].equals("P")) {
					Patient curPatient = new Patient(curData[1], curData[2], curData[3]);
					patients.put(curData[3], curPatient);
					correctlyReadLines++;
				} else if (curData[0].equals("M")) {
					Doctor curDoctor = new Doctor(curData[2], curData[3], curData[4], Integer.valueOf(curData[1]), curData[5]);
					doctors.put(Integer.valueOf(curData[1]), curDoctor);
					correctlyReadLines++;
				}
			} catch (Exception e) {
	...
	public Collection<String> doctorsByNumPatients(){
		List<String> result = doctors.values().stream()
			.sorted(Comparator.comparing(d -> d.assignedPatients.size(),Comparator.reverseOrder()))
			.map(d -> {
				try {
					return d.getPatientSize() + " : " + getDoctor(d.docID);
