...
public class Region {
	...
	public MountainHut createOrGetMountainHut(String name, String category, Integer bedsNumber,Municipality municipality) {
		mountainhuts.putIfAbsent(name, new MountainHut(name, category, bedsNumber, municipality));
		...
		readData(file).stream().skip(1).map(x->x.split(";")).forEach(data -> 
							{
								...
								if(data[4] == ""){
									curRegion.createOrGetMountainHut(data[3],data[5],Integer.parseInt(data[6]),curMunicipality);
								}
								else{
    ...
    public Map<Long, List<String>> municipalityNamesPerCountOfMountainHuts() {
        // Step 1: Count the number of huts per municipality
        Map<String, Long> municipalityHutCounts = mountainhuts.values()
            .stream()
            .collect(Collectors.groupingBy(hut -> hut.getMunicipality().getName(), Collectors.counting()));

        // Step 2: Invert the map to group municipality names by the count of huts
        Map<Long, List<String>> result = municipalityHutCounts.entrySet()
            .stream()
            .collect(Collectors.groupingBy(
                Map.Entry::getValue,
        ...
        // Step 3: Sort the municipality names in each list alphabetically
